...
import numpy as np
import torch
# matplotlib imported lazily inside functions (avoids hard dep for headless use)
from os import makedirs
from os.path import join, exists

from .ply import read_ply, write_ply
from .printing import bcolors
from .rotation import create_3D_rotations
from .gpu_init import init_gpu
...
def load_kernels(radius, shell_sizes, dimension, fixed, lloyd=False):
    ...
    # Kernel directory (configurable, repo-local — not CWD-relative)
    import os
    kernel_dir = os.environ.get('KPCONVX_KERNEL_DIR') or os.path.join(os.path.dirname(os.path.abspath(__file__)), 'dispositions')
