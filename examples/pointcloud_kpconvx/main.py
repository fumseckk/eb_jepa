...
def build_encoder(cfg):
    """Build the point-cloud encoder mapping [B, 3, N] -> [B, D].

    Branches on ``cfg.encoder``: ``"kpconvx"`` uses the vendored KPConvX encoder
    (apple/ml-kpconvx, CVPR 2024) — kernel-point convolution with a pure-torch
    neighborhood pyramid. The legacy ``"pointnet"`` fallback is kept for
    backward compatibility. Exposes `.represent(x) -> [B, D]` and `.out_dim`.
    """
    encoder_type = str(getattr(cfg, "encoder", "pointnet")).lower()
    in_channels = int(getattr(cfg, "in_channels", 3))
    out_dim = int(getattr(cfg, "out_dim", 1024))

    if encoder_type == "kpconvx":
        from examples.pointcloud_kpconvx.kpconvx import KPConvXEncoder
        kw = OmegaConf.to_container(getattr(cfg, "kpconvx", {}), resolve=True) or {}
        return KPConvXEncoder(in_channels=in_channels, out_dim=out_dim, **kw)

    # ---- legacy PointNet fallback (unchanged) ----
...
def evaluate_probe(encoder, cfg, device):
    from examples.pointcloud_kpconvx.eval import extract_features, probe, build_random_encoder
...
# --------------------------------------------------------------------------- #
def run(fname="examples/pointcloud_kpconvx/cfgs/train.yaml", cfg=None, folder=None, **overrides):
    if cfg is None:
    ...
    wandb_run = setup_wandb(
        project="eb_jepa",
        config={"example": "pointcloud_kpconvx", **OmegaConf.to_container(cfg, resolve=True)},
        run_dir=folder or cfg.meta.ckpt_dir,
        run_name=f"pointcloud_kpconvx_{cfg.data.rotate}",
        tags=["pointcloud", "kpconvx", f"rotate_{cfg.data.rotate}", f"seed_{cfg.meta.seed}"],
    ...
    for epoch in range(cfg.optim.epochs):
        ...
        for batch in loader:
            ...
            n_batches += 1
        loss_val = epoch_loss / max(n_batches, 1)
        print(f"[pointcloud_kpconvx:{cfg.data.rotate}] epoch {epoch} loss={loss_val:.4f} {logs}", flush=True)
        ...
        if eval_every > 0 and (epoch % eval_every == 0 or epoch == cfg.optim.epochs - 1):
            ...
            print(f"[pointcloud_kpconvx:{cfg.data.rotate}] epoch {epoch} eval={eval_logs}", flush=True)
            print(f"[pointcloud_kpconvx:{cfg.data.rotate}] epoch {epoch} probe={probe_logs}", flush=True)
        if wandb_run:
            import wandb

            log_dict = {"epoch": epoch, **{f"train/{k}": v.item() if torch.is_tensor(v) else v for k, v in logs.items()}, "train/loss": loss_val}
    ...
    if wandb_run:
        ...
        wandb.finish()
    print(f"[pointcloud_kpconvx] done -> {ckpt_dir}/latest.pth.tar")


if __name__ == "__main__":
    fname = sys.argv[sys.argv.index("--fname") + 1] if "--fname" in sys.argv \
        else "examples/pointcloud_kpconvx/cfgs/train.yaml"
    run(fname=fname)
